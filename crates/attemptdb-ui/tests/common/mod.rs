...
pub const ROOT: &str = "/home/alice/work/repo";
...
/// The reference story plus one absolute path outside the repository (a
/// home-directory file read), so exports can be checked for leaks.
pub fn ui_scenario() -> Scenario {
    let mut sc = spec_scenario();
    let outside = PortablePath::from_raw("/home/alice/.config/notes.txt", Some(ROOT));
    let mut patched = 0;
    for ev in sc.events.iter_mut() {
        if ev.tool.as_ref().is_some_and(|t| t.name == "Read") {
            ev.paths.push(outside.clone());
            patched += 1;
        }
    }
    assert_eq!(
        patched, 2,
        "the Read start and end events carry the outside path"
    );
    sc
}
