...
fn exported_prompt_and_reply_become_content_under_the_capture_mode_never_metadata() {
    let prompt = logs(
        ...
        vec![
            ...
            attr("prompt", json!({"stringValue":"make the retries idempotent"})),
        ],
    );
    let reply = logs(
        "claude-code",
        ...
        vec![
            ...
            attr("response", json!({"stringValue":"I will read the webhook handler first."})),
            attr("model", json!({"stringValue":"claude-sonnet-4-6"})),
        ],
    );
    let redacted = logs(
    ...
    let e = &normalise(&ctx, Provider::ClaudeCode, Signal::Logs, &prompt).unwrap().events[0];
    assert_eq!(e.content.as_ref().unwrap().prompt.as_deref(), Some("make the retries idempotent"));
    assert_eq!(e.attrs["x_otel_prompt_chars"], 27);
    assert!(!serde_json::to_string(&e.attrs).unwrap().contains("idempotent"));
    assert!(e.is_telemetry());
    let e = &normalise(&ctx, Provider::ClaudeCode, Signal::Logs, &reply).unwrap().events[0];
    assert_eq!(
    ...
    let e = &normalise(&ctx, Provider::ClaudeCode, Signal::Logs, &redacted).unwrap().events[0];
    assert!(e.content.is_none());
    assert_eq!(e.attrs["x_otel_response_chars"], 38);
    // Metadata-only capture: the text never lands anywhere.
    let ctx = context(CaptureMode::MetadataOnly);
    for payload in [&prompt, &reply] {
        let e = &normalise(&ctx, Provider::ClaudeCode, Signal::Logs, payload).unwrap().events[0];
