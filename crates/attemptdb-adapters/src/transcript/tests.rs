...
fn every_event_is_marked_reconstructed() {
    for name in FIXTURES {
        ...
        let session = import.provider_session_id.clone().expect("session id found");
        ...
        for ev in &import.events {
            ...
            assert!(ev.provider_event_name.starts_with("transcript:"), "{label}: name");
            assert_eq!(attr(ev, "reconstructed"), &Value::Bool(true), "{label}");
            assert_eq!(attr(ev, "reconstructed_from"), "claude_code_transcript", "{label}");
            assert_eq!(attr(ev, "transcript_present"), &Value::Bool(true), "{label}");
            assert!(attr(ev, "transcript_entry_type").is_string(), "{label}: entry type");
            assert!(ev.attrs.get("hook_event_name").is_none(), "{label}: hook_event_name");
            assert_ne!(ev.observed_at, Timestamp::from_micros(0), "{label}: observed_at");
            for key in ev.attrs.keys() {
                assert!(ALLOWED_ATTR_KEYS.contains(&key.as_str()), "{label}: attr `{key}` not allowlisted");
            }
        }
    }
}
...
fn basic_turn_kinds_and_order() {
    ...
    let s = &import.stats;
    assert_eq!((s.entries, s.prompts, s.tool_calls, s.tool_failures, s.turns), (19, 2, 2, 1, 2));
    assert_eq!((s.unknown_entries, s.malformed_lines, s.subagent_entries), (0, 0, 0));
    assert_eq!(s.skipped_entries, 10);
    assert_eq!(import.provider_session_id.as_deref(), Some("11111111-1111-4111-8111-111111111111"));
    ...
    assert_eq!(start.observed_at, Timestamp::parse("2026-08-20T09:00:00.000Z").unwrap());
    ...
    assert_eq!(prompt.provider_turn_id.as_deref(), Some("p0000001-0000-4000-8000-000000000000"));
    ...
    assert!(prompt.content.as_ref().unwrap().prompt.as_deref().unwrap().starts_with("CANARY_PROMPT_ONE"));
    assert_eq!(prompt.project.branch.as_deref(), Some("main"), "gitBranch fills the branch the context lacks");
    assert!(import.events.iter().all(|e| e.provider_version.as_deref() == Some("2.1.190")));
}
...
fn basic_turn_project_branch_comes_from_transcript_when_ctx_lacks_it() {
    ...
    assert!(import.events.iter().all(|e| e.project.branch.as_deref() == Some("main") || e.project.branch.is_none()));
    assert!(import.events.iter().any(|e| e.project.branch.as_deref() == Some("main")));
    let mut ctx = context(CaptureMode::LocalSemantic);
    ctx.project.branch = Some("from-ctx".into());
    let import = parse_claude_transcript(lines("basic_turn").into_iter(), &ctx, &options_for("basic_turn", true));
    assert!(import.events.iter().all(|e| e.project.branch.as_deref() == Some("from-ctx")));
}
...
fn tool_results_pair_with_calls() {
    ...
    let bash_start = &ev[2];
    let bash_end = &ev[3];
    assert_eq!(bash_start.provider_event_name, "transcript:assistant:tool_use");
    assert_eq!(bash_end.provider_event_name, "transcript:user:tool_result");
    let t = bash_start.tool.as_ref().unwrap();
    assert_eq!((t.name.as_str(), t.category, t.call_id.as_deref()), ("Bash", ToolCategory::Shell, Some("toolu_0001")));
    ...
    assert_eq!(bash_start.content.as_ref().unwrap().command.as_deref(), Some("cargo test -p example"));
    assert_eq!(bash_end.outcome.as_ref().map(|o| o.status), Some(OutcomeStatus::Success));
    assert!(bash_end.content.as_ref().unwrap().tool_output.as_ref().unwrap().as_str().unwrap().starts_with("CANARY_TOOL_OUTPUT"));
    assert_eq!(bash_end.observed_at, Timestamp::parse("2026-08-20T09:00:05.000Z").unwrap());

    let edit_start = &ev[4];
    let edit_end = &ev[5];
    assert_eq!(edit_start.tool.as_ref().unwrap().call_id.as_deref(), Some("toolu_0002"));
    assert_eq!(edit_start.paths.iter().map(|p| p.display()).collect::<Vec<_>>(), vec!["src/lib.rs"]);
    ...
    assert_eq!((o.status, o.class.as_deref()), (OutcomeStatus::Failure, Some("string_mismatch")));
    assert_eq!(attr(edit_end, "error_class"), "string_mismatch");
    assert_eq!(edit_end.tool.as_ref().unwrap().name, "Edit");
    assert_eq!(edit_end.paths.iter().map(|p| p.display()).collect::<Vec<_>>(), vec!["src/lib.rs"]);
    assert!(edit_end.content.as_ref().unwrap().error.as_deref().unwrap().contains("String to replace not found"));
...
fn turn_end_is_synthesised_before_next_prompt_and_at_eof() {
    ...
    assert_eq!(message.observed_at, Timestamp::parse("2026-08-20T09:00:09.000Z").unwrap());
    assert_eq!(stop.duration_ms, Some(9000), "durationMs of the turn_duration entry");
    assert_eq!(provider_attr(stop, "stop_reason"), "end_turn");
    let text = message.content.as_ref().unwrap().message.clone().unwrap();
    assert_eq!(provider_attr(message, "message_chars"), text.chars().count() as u64);
    assert_eq!(provider_attr(message, "output_tokens"), 42);
    assert_eq!(message.agent.model.as_deref(), Some("claude-opus-4-8"));
    assert!(message.content.as_ref().unwrap().message.as_deref().unwrap().starts_with("CANARY_FINAL_MESSAGE"));
    assert!(message.attrs.get("message_chars").is_none(), "message_chars lives under attrs.provider");
    // Interim narration followed by a tool call is not a message.
    assert!(!ev.iter().any(|e| serde_json::to_string(e).unwrap().contains("CANARY_INTERIM_TEXT")));

    let last_stop = ev.last().unwrap();
    assert_eq!(last_stop.kind, TurnStopped);
    assert_eq!(last_stop.observed_at, Timestamp::parse("2026-08-20T09:01:03.000Z").unwrap());
    assert_eq!(last_stop.duration_ms, None);
    assert_eq!(attr(last_stop, "turn_index_hint"), 2);
    assert_eq!(last_stop.provider_turn_id.as_deref(), Some("p0000002-0000-4000-8000-000000000000"));
}
...
fn growing_transcript_only_adds_events() {
    ...
    for ev in &before.events {
        assert!(after_ids.contains(&ev.event_id), "{} vanished on re-import", ev.provider_event_name);
        let same = after.events.iter().find(|e| e.event_id == ev.event_id).unwrap();
        assert_eq!(zeroed_for_golden(ev.clone()), zeroed_for_golden(same.clone()));
    }
}
...
fn sidechain_entries_are_attributed_to_the_subagent() {
    ...
    assert_eq!(
        kinds(&import),
        vec![SubagentStarted, ToolCallStarted, ToolCallFinished, AgentMessage, SubagentStopped]
    );
    ...
    assert_eq!(start.observed_at, Timestamp::parse("2026-08-20T09:00:20.000Z").unwrap());
    assert!(start.content.as_ref().unwrap().prompt.as_deref().unwrap().starts_with("CANARY_SUBAGENT_TASK"));
    ...
    assert_eq!(stop.observed_at, Timestamp::parse("2026-08-20T09:00:24.000Z").unwrap());
    let grep = &import.events[1];
    assert_eq!(grep.tool.as_ref().unwrap().category, ToolCategory::Search);
    assert_eq!(grep.paths.iter().map(|p| p.display()).collect::<Vec<_>>(), vec!["src"]);
}
...
fn compaction_markers_become_compaction_finished() {
    ...
    assert_eq!(summary.content.as_ref().unwrap().extra.get("summary").and_then(Value::as_str).map(|s| s.starts_with("CANARY_OLD_SUMMARY")), Some(true));
    ...
    assert_eq!(boundary.provider_event_name, "transcript:system:compact_boundary");
    ...
    assert_eq!(boundary.observed_at, Timestamp::parse("2026-08-21T10:05:00.000Z").unwrap());
    ...
    assert!(import.events.iter().enumerate().all(|(i, e)| i == 1 || e.project.branch.as_deref() == Some("feature/compaction")));
...
fn interrupted_turns_are_cancelled_stops() {
    ...
    assert_eq!((o.status, o.class.as_deref()), (OutcomeStatus::Denied, Some("user_rejected")));
    ...
    assert_eq!((o.status, o.class.as_deref()), (OutcomeStatus::Cancelled, Some("interrupted")));
    ...
    assert!(partial.content.as_ref().unwrap().message.as_deref().unwrap().starts_with("CANARY_PARTIAL_MESSAGE"));
    assert!(partial.attrs.get("provider").and_then(|p| p.get("stop_reason")).is_none());
    let stop_turn = &import.events[7];
    assert_eq!(provider_attr(stop_turn, "interrupt_kind"), "turn");
    assert_eq!(stop_turn.observed_at, Timestamp::parse("2026-08-22T11:00:22.000Z").unwrap());
...
fn unknown_entries_malformed_lines_and_in_flight_calls() {
    ...
    let s = &import.stats;
    assert_eq!((s.malformed_lines, s.unknown_entries, s.tool_calls, s.subagent_entries), (2, 2, 2, 4));
    assert_eq!(s.turns, 0, "a tool call left in flight synthesises no turn end");
    ...
    assert_eq!(inline_start.agent.provider_agent_id.as_deref(), Some("sidechain:e0000004-0000-4000-8000-000000000000"));
    assert_eq!(inline_start.agent.agent_type, None);
    assert!(inline_start.content.as_ref().unwrap().prompt.as_deref().unwrap().starts_with("CANARY_INLINE_TASK"));
    ...
    assert_eq!(main_call.tool.as_ref().unwrap().call_id.as_deref(), Some("toolu_m003"));
    assert_eq!(attr(main_call, "git_subcommand"), "status");
    let inline_stop = &import.events[9];
    assert_eq!(inline_stop.observed_at, Timestamp::parse("2026-08-23T12:00:04.000Z").unwrap());
...
fn session_id_falls_back_to_the_hint() {
    ...
    assert_eq!(import.provider_session_id.as_deref(), Some("basic_turn-stem"));
    assert!(import.warnings.iter().any(|w| w.contains("using the file name")));
    assert_eq!(import.events.len(), 11);
    assert!(import.events.iter().all(|e| e.provider_session_id == "basic_turn-stem"));
    ...
    assert_eq!(none.events.iter().map(|e| e.kind).collect::<Vec<_>>(), vec![SessionStarted, PromptSubmitted]);
}
...
fn golden_envelopes_match() {
    ...
    for name in FIXTURES {
        ...
        let golden: Value = serde_json::from_str(&fs::read_to_string(&golden_path).expect("read golden"))
        ...
        for item in golden.as_array().expect("golden is an array") {
            ...
            assert!(back.unknown.is_empty(), "{}: unknown fields", golden_path.display());
        }
    }
    assert!(
        mismatches.is_empty(),
...
fn metadata_only_output_carries_no_content() {
    for name in FIXTURES {
        for (mode, include) in [
            ...
            for ev in &import.events {
                assert!(ev.content.is_none(), "{name}: content present ({mode}, include_content={include})");
            ...
            assert!(!serialised.contains("must-not-appear"), "{name}: hook version leaked");
            assert!(!serialised.contains(".jsonl"), "{name}: transcript path leaked");
        }
    }
}
        ...
        for ev in &import.events {
            ...
            if let Some(provider) = ev.attrs.get("provider") {
                for (k, v) in provider.as_object().unwrap() {
                    assert!(
                        v.is_number() || v.is_boolean() || v.as_str().is_some_and(|s| s.len() <= 64),
                        "{name}: provider attr `{k}` is not a short scalar"
                    );
                }
            }
        }
        assert!(saw_content, "{name}: expected some content in local_semantic mode");
    }
}
