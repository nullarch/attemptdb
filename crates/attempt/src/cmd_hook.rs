...
fn hook_entry(cli: &Cli, provider_id: &str, event_hint: Option<&str>) -> Result<ExitCode> {
    if AgentKind::from_provider_id(provider_id).is_none() && provider_id.parse::<Provider>().map(|p| matches!(p, Provider::Other(_))).unwrap_or(true) {
...
fn install_cmd(cli: &Cli, args: &HookArgs, remove: bool) -> Result<ExitCode> {
    ...
    let providers = if args.providers.is_empty() {
        ...
        for p in &args.providers {
            kinds.push(AgentKind::from_provider_id(p).with_context(|| format!("unknown provider id {p:?} (expected claude-code, codex, cursor, gemini-cli)"))?);
        }
        Some(kinds)
    };
    let opts = InstallOptions { scope, providers, binary_path: None, dry_run: args.dry_run };
    let report = if remove { uninstall(&opts)? } else { install(&opts)? };
    if cli.json {
        print_json(&report);
    } else {
        if report.actions.is_empty() {
            println!("no coding agents detected (looked for Claude Code, Codex, Cursor, Gemini CLI)");
        }
    ...
    let failed = report.actions.iter().any(|a| matches!(a.outcome, Outcome::Failed(_)));
...
fn verify_capture(cli: &Cli, ctx: &Ctx, report: &attemptdb_capture::install::InstallReport) -> Result<()> {
    if !Database::exists(&ctx.locator.db_dir) {
        println!("\ndatabase not initialised yet at {} — run `attempt init` to start capturing", ctx.locator.db_dir.display());
        return Ok(());
    }
    let mut tested = 0;
    for a in &report.actions {
        if !matches!(a.outcome, Outcome::Installed | Outcome::Updated | Outcome::AlreadyCurrent) {
            continue;
        }
    ...
    if tested > 0 {
        ...
        println!("\ncapture test: {tested} event(s) went through the hook pipeline into {}", ctx.locator.db_dir.display());
...
pub fn doctor(cli: &Cli) -> Result<ExitCode> {
        ...
        match ctx.open(cli) {
            Ok(opened) => {
                ...
                db_line = format!(
                    ...
                    stats.segment_rows,
                    stats.memtable_rows,
                    if opened.read_only { ", read-only: another writer holds the lock" } else { "" }
                );
                let events = opened.db.scan(&ScanFilter::default())?;
                for ev in &events {
                    let Some(kind) = AgentKind::from_provider_id(&ev.provider.as_str().replace('_', "-")) else { continue };
            ...
            Err(e) => db_line = format!("database     {} — cannot open: {e}", ctx.locator.db_dir.display()),
        }
    } else {
        db_line = format!("database     not initialised at {} — run `attempt init`", ctx.locator.db_dir.display());
    }
    let diag = diagnose(&|kind| activity.get(&kind).cloned());
    if cli.json {
        print_json(&serde_json::json!({ "diagnosis": diag, "database": db_line, "capture_mode": ctx.config.capture_mode.as_str() }));
        return Ok(ExitCode::SUCCESS);
    }
    println!("attempt {}", env!("CARGO_PKG_VERSION"));
    println!("binary       {}{}", diag.binary.display(), if diag.binary_on_path { "" } else { "  (not on PATH)" });
    println!("{db_line}");
    ...
    for a in &diag.agents {
        ...
        let act_s = match act {
            Some(s) if s.event_count > 0 => format!("{} events, last {}", s.event_count, s.last_event_at.clone().unwrap_or_default()),
            Some(s) if s.capture_test_seen => "capture test ok, no agent events yet".to_string(),
            _ => "no events yet".to_string(),
        };
        println!("{:<12} {:<13} {}  [{}]", a.agent.display_name(), state, a.config_path.display(), act_s);
        for n in &a.notes {
            println!("{:<12} note: {n}", "");
        }
    ...
    Ok(if problems > 0 { ExitCode::from(1) } else { ExitCode::SUCCESS })
}
